...
class classification_GPCNN():
    ...
    def _create_model(self, print_sum=True):
        ...
        def make_prior_fn(dtype=tf.float32, shape=None, name=None, trainable=False, add_variable_fn=None):
            prior_loc = tf.zeros(shape, dtype=dtype)
            prior_scale = 1.0
            return tfp.distributions.Normal(loc=prior_loc, scale=prior_scale, name=name)

        def make_posterior_fn(dtype=tf.float32, shape=None, name=None, trainable=True, add_variable_fn=None):
            dtype = tf.cast(dtype, dtype=tf.float32)
            posterior_loc = tf.Variable(
                tf.random.normal(shape, dtype=dtype),
                trainable=trainable,
                name=name + "_loc"
            )
            posterior_scale = tfp.layers.VariableLayer(
                tfp.distributions.Softplus()(tf.Variable(
                    tf.random.normal(shape, dtype=dtype),
                    trainable=trainable,
                    name=name + "_scale"
                )),
            ...
            return tfp.distributions.Normal(loc=posterior_loc, scale=posterior_scale, name=name)

        dense_variational_layer = tfp.layers.DenseVariational(
        ...
        output = dense_variational_layer(cnn_output)
