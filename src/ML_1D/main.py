...
def train_regression(data_sets, input_shape, num_classes, model_prefix):
    """
    ...
    model = regression_CNN(X_train, X_train_cl, y_train, X_test, X_test_cl, y_test, input_shape, num_classes, model_name)
    model.compile()
    model.train()
...
def val_regression(data_sets, input_shape, num_classes, model, img_save_path):
    """
    ...
    X_val, X_val_cl, y_val = data_sets
    model.evaluate_model(X_val, X_val_cl, y_val, img_save_path)
...
def main(run_mode, model_type, model_prefix):
    ...
    data_sets, input_shape = get_sets(sneutrino_jet_path, neutralino_jet_path) 
    ...
    re_data_set = [X_train, y_train_cl, y_train_re, X_test, y_test_cl,  y_test_re]

    cl_data_set_val = [X_val, y_val_cl]
    re_data_set_val = [X_val, y_val_cl,  y_val_re]
    ...
    if model_type == "cl":
        ...
        re_model = train_regression(re_data_set, input_shape, num_classes, model_prefix) 
